...
namespace esp32_thermostat
{
  ...
  bool item_overflow()
  {
    if ((id(selected_display_page) == SETPOINT_SCREEN) && (id(esp_thermostat).mode != CLIMATE_MODE_HEAT_COOL))
  ...
  float adjust_high_set_point(const float adjustment)
  {
    id(esp_thermostat).target_temperature_high += adjustment;
    id(esp_thermostat).validate_target_temperature_high();

    return id(esp_thermostat).target_temperature_high;
  }

  float adjust_low_set_point(const float adjustment)
  {
    id(esp_thermostat).target_temperature_low += adjustment;
    id(esp_thermostat).validate_target_temperature_low();
  ...
  void encoder_value_change()
  {
    ...
    if ((id(selected_display_page) == SETPOINT_SCREEN) || (id(selected_display_page) == MODE_SCREEN))
    {
      ...
      {
        if (supported_fan_modes[i] == id(esp_thermostat).fan_mode.value_or(CLIMATE_FAN_AUTO))
          fan_mode_selected = i;
      }
  ...
  void draw_main_screen(DisplayBuffer *it)
  {
    ...
    else
    {
      ...
      switch (id(esp_thermostat).fan_mode.value_or(CLIMATE_FAN_AUTO))
      {
      case CLIMATE_FAN_ON:
      ...
      if ((id(esp_thermostat).fan_mode.value_or(CLIMATE_FAN_AUTO) != CLIMATE_FAN_OFF) && (id(esp_thermostat).fan_mode.value_or(CLIMATE_FAN_AUTO) != CLIMATE_FAN_AUTO))
        it->printf(0, 9, thermostat_small, color_action, TextAlign::TOP_LEFT, status_string);

      if (id(esp_thermostat).climate_action_change_delayed() || id(esp_thermostat).fan_mode_change_delayed())
      {
        it->printf(0, it->get_height() - (line_height_tiny * 2), thermostat_tiny, color_footer, TextAlign::BASELINE_LEFT, "Delay");
      }
